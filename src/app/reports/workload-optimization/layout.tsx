  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Workload Optimization</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Resource utilization and efficiency opportunities &middot;{" "}
            <span className="font-mono text-xs">{filteredFacts.length.toLocaleString()} records</span>
          </p>
        </div>
        <ExportButton data={filteredFacts} filename="workload-optimization" />
      </div>
      <FilterBar
...
export default function WorkloadOptimizationLayout({ children }: { children: ReactNode }) {
  return (
    <ReportProvider>
